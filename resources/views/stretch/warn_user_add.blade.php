...
@section('container')
    ...
    <form class="form form-horizontal " id="form_container" data-url="{{url('stretch/warn_user_add')}}">
                ...
                <select class="select msg_must" id="user_id" name="user_id" placeholder="请选择">
                    ...
                    @if($user_data)
                      @foreach($user_data as $user)
                            <option value="{{$user->id}}">{{$user->name}}/{{$user->roled->display_name}}/{{$user->posi->name}}</option>
                      @endforeach
                    @endif
                </select>
            </span>

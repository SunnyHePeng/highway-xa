    		...
    		<span style="float:left;padding: 6px 10px;">选择设备</span>
		  	<span class="col-sm-11 col-xs-8" style="padding: 3px 5px;">
		  		<select name="dev_id[]" id="dev_id" multiple class="select select2">
	                ...
	                <option value="{{$v['id']}}">{{$v['model']}}{{$v['category']['name']}}</option>
	                @endforeach
	                @endif
	            </select>
	        </span>
	...
	<table id="table_list" class="table table-border table-bordered table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="100">报警等级</th>
				<th width="150" class="hidden-xs">试验时间</th>
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$value['device']['model']}}{{$value['device']['category']['name']}}</td>
				<td>{{$value['syzh']}}</td>
				<td>{{$value['warn_info']}}</td>
				<td>
					@if($value['warn_level'])
						@if($value['warn_level']==1)
							初级
						@elseif($value['warn_level']==2)
							中级
						@elseif($value['warn_level']==3)
                            高级
						@endif
					@else
						高级
				...
				<td class="f-14 product-brand-manage td-manage">
					@if($value['reportFile'])
					<a style="text-decoration:none" class="mt-5 ml-5 btn btn-secondary radius size-MINI" href="{{$value['reportFile']}}" target="_blank" data-title="试验报告">试验报告</a> 
				  	@endif
				  	<a style="text-decoration:none" class="mt-5 ml-5 open-iframe btn btn-secondary radius size-MINI" href="javascript:;" data-title="{{$value['section']['name']}}-{{$value['device']['name']}} 试验数据" data-url="{{url('lab/lab_data_detail?id='.$value['id'])}}">试验数据</a>
          			<a style="text-decoration:none" class="mt-5 ml-5 open-iframe btn btn-secondary radius size-MINI" href="javascript:;" data-title="处理报告"  data-url="{{url('lab/clbg?id='.$value['id'])}}">处理报告</a>
          		</td>
			</tr>
			@endforeach
			@endif
...
var url = "{{url('lab/get_select_val')}}";
//项目改变 获取对应的监理 标段 设备

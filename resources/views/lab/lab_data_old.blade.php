...
<div class="mt-10 dataTables_wrapper">
	<table id="table_list" class="table table-border table-bordered table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<?php 
					$status_info = json_decode(Cache::get('device_status_'.$value['id']), true);
				...
				<td class="f-14 product-brand-manage td-manage"> 
				  	<a style="text-decoration:none" class="mt-5 ml-5 open-iframe btn btn-secondary radius size-MINI" href="javascript:;" data-title="{{$value['project']['name']}}-{{$value['model']}}{{$value['category']['name']}} 试验数据" data-url="{{url('lab/lab_data_info/'.$value['id'].'?pro_id='.$value['project_id'])}}">试验数据</a>
          		</td>
			</tr>
			@endforeach
			@endif

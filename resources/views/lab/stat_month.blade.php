...
<div>
  <form id="search_form" data="d_id,start_date,end_date" method="get" data-url="{{url('lab/stat_month')}}">
        设备名称
      	<span class="select-box" style="width:auto; padding: 3px 5px;">
        	<select name="d_id" id="d_id" class="select select2">
                @if(isset($device))
                @foreach($device as $v)
                <option value="{{$v['id']}}" @if(isset($search['d_id']) && $search['d_id'] == $v['id']) selected @endif>{{$v['model']}}-{{$v['category']['name']}}</option>
                @endforeach
                @endif
            </select>
        </span>
...
<div class="cl pd-5 bg-1 bk-gray mt-20 hidden-xs"> 
  <span class="dropDown"> <a class="btn btn-success radius" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">导出/打印</a>
    <ul class="dropDown-menu menu radius box-shadow">
      <li><a href="#" class="export-cur open-iframe" data-title="试验室月报表" data-url="{{$url.'&d=cur'}}">当前页面数据</a></li>
      <li><a href="#" class="export-all open-iframe" data-title="试验室月报表" data-url="{{$url.'&d=all'}}">全部页面数据</a></li>
    </ul>
  </span>
</div>

...
@section('container')
    <div class="col-xs-12 col-sm-12  mt-20" style="padding-left: 30px;">
  <span class="l">
    <a class="btn open-add btn-primary radius" data-is-reload="1" data-title="添加微信推送通知人员" data-url="{{url('notice/add_user')}}" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i>添加微信推送通知人员</a>
  </span>
    </div>
                    ...
                    @if($data)
                        @foreach($data as $k=>$v)
                            <tr class="text-c">
                                ...
                                <td>{{$v['user']['phone']}}</td>
                                <td>
                                    <a style="text-decoration:none" class="ml-5 del-sd btn btn-danger radius size-MINI" href="javascript:;" data-url="{{url('notice/del_user').'?id='.$v['id']}}" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                                </td>
                            </tr>
                        @endforeach
                    @endif

        ...
        @if($device_data)
            @foreach($device_data as $k=>$v)
                <tr class="text-c">
                    ...
                    <td>
                        <input class="btn btn-primary radius size-S ml-10 open-data" data-title="压浆数据" data-url="{{url('mudjack/mudjack_data_by_device'.'/'.$v->id)}}" type="button" value="压浆数据">
                        <input class="btn btn-success radius size-S ml-10 open-video" data-title="实时视频" data-url="{{url('mudjack/real_time_video'.'/'.$v->id)}}" type="button" value="实时视频">
                    </td>
                </tr>
            @endforeach
        @endif

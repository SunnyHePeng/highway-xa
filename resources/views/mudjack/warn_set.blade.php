...
@section('container')
  ...
  <span class="l">
    <a class="btn open-data btn-primary radius" data-is-reload="1" data-title="添加报警通知人员" data-url="{{url('mudjack/warn_user_add'.'?sec_id='.$search['search_sec_id'])}}" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i>添加报警通知人员</a>
  </span>
    </div>
            ...
            @if($warn_users)
                @foreach($warn_users as $k=>$user)
                    <tr class="text-c" id="{{'list-'.$user->id}}">
                        ...
                        <td>
                            <a style="text-decoration:none" class="ml-5 del-r btn btn-danger radius size-MINI" data-id="{{$user->id}}" href="javascript:;" data-url="{{url('mudjack/warn_user_del'.'/'.$user->id)}}" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
                    </tr>
                @endforeach
            @endif

            ...
            @if($data)
                @foreach($data as $v)
                    <tr  class="text-c">
                        ...
                        <td>
                            <input class="btn btn-success radius size-MINI mr-10 open-data" data-title="详细数据" data-url="{{url('mudjack/detail_in_warn'.'/'.$v->detail_id)}}" type="button" value="详细数据">
                            <input class="btn btn-primary radius size-MINI open-data" type="button" data-title="处理报告" data-url="{{url('mudjack/warn_deal_report'.'/'.$v->detail_id)}}" value="处理报告">
                        </td>
                    </tr>
                @endforeach
            @endif

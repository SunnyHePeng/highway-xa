...
// 设置预览主子码流类型
function SetTreamType(){
	var vStreamType = document.getElementById("StreamTypeId").value;
	var OCXobj = document.getElementById("PreviewOcx");
	var ret = OCXobj.SetInitStreamType(vStreamType);
	if (vStreamType == 1){
		if (0 == ret){
			//alert("设置主码流成功！");
		}
...
function loginCMSEx(){
	var OCXobj = document.getElementById("PreviewOcx");
	var result = OCXobj.SetLoginType(1);    // 设置异步登入模式
	if (0 != result){
		//alert("设置异步登入模式失败！");
		return ;
	}
	...
	switch(ret){
	    case 0:
	        clearTree();
	        //alert("异步登录成功，请等待加载数据结果通知后再进行其它操作！");
	        showMethodInvokedInfo("异步Login接口调用成功！");
	        break;
		case -1:
			clearTree();
			//alert("异步登录失败！");
			showMethodInvokedInfo("异步Login接口调用失败！错误码："+ OCXobj.GetLastError());
			break;
		default:
			break;
	}
...
function initCameraList(){
	...
	$(xmldom).find("CameraInfo").each(function() {
        if("|2621|2622|2623|2624|2625|2661|".indexOf("|"+$(this).find("CameraID").text()+"|")==-1) return;
	htmlStr += "<li onclick='startPreview(" + $(this).find("CameraID").text() + ");' onMouseDown='setCameraIndexCode(\"" + $(this).find("CameraIndexCode").text() + "\");' onMouseUp='setCameraId(" + $(this).find("CameraID").text() + ");'><a href='#' style='text-decoration:none'>" + $(this).find("CameraName").text() + "</a></li>" ;
    });
	$("#tree").html(htmlStr); 
}
...
function initTree(){
	...
	if(orderdCellIds.length > 0){
       ...
       quanXianSet();//权限设置，相应的用户看到相应的视频监控	
...
function quanXianSet() {
    $(".region_li ul").each(function () {
        if ($(this)[0].innerText == "") {
            $(this).parent().remove();
        }
    });

    $(".control_unit_li ul").each(function () {
        if ($(this)[0].innerText == "") {
            $(this).parent().remove();
            quanXianSet();
        }
...
function initCameraNodes(xmlStr, regionIds){
	...
	var vbCheckRight = document.getElementById("rightSelId").value;
	if (vbCheckRight == 2)
	{
		var vTotalPage = parseInt(cameras.length / 1000 + 1);
		var vLastPageSize = cameras.length % 1000;
		var OCXobj = document.getElementById("PreviewOcx");
		for (var vPage = 0; vPage < vTotalPage; ++vPage)
		{
			...
			// 先将一页数据存map表，并拼接好查询字符串
			for (var iCount = 0; iCount < vPageSize; ++iCount)
			{
				var vIndex = vPage*1000 + iCount;
				var vIdx = $(cameras[vIndex]).find("CameraIndexCode").text();
				if (iCount == 0)
				{
					camIdxs = vIdx;
				}
				...
				{
					camIdxs = camIdxs + ',' + vIdx;
				}
				vMap[vIdx] = vIndex;    // first-监控点编号 second-监控点在cameras中的序号
			}
			
			// 批量查询预览权限
			var vXml = OCXobj.QueryRightByIndexCodes(128, camIdxs);
			if (vXml == null)
			{
				//alert("查询预览权限失败！");
				return ;
			...
			{
				vXmldom = new ActiveXObject("Microsoft.XMLDOM");
				vXmldom.loadXML(vXml);
			}
			else
			{
				vXmldom = new DOMParser().parseFromString(vXml,"text/xml");
			}
			
			// 权限过滤
			var vChildNodesLen = vXmldom.getElementsByTagName("RightInfo")[0].childNodes.length;
			for (var j = 0; j < vChildNodesLen; ++j)
			{
				var vCheckedIdx= vXmldom.getElementsByTagName("right")[j].getAttribute("indexCode");
				var vHasRight = vXmldom.getElementsByTagName("right")[j].getAttribute("rightCode");
				if (vCheckedIdx && (vHasRight == 0))
				{
					if (vCheckedIdx in vMap)
					{
						var vMapIndex = vMap[vCheckedIdx];
						
						expr = "#ul_region_" + $(cameras[vMapIndex]).find("RegionID").text();
						$(expr).append("<li class='camera_li' ondblclick='startPreview(" + $(cameras[vMapIndex]).find("CameraID").text() + ");' onMouseDown='setCameraIndexCode(\"" + $(cameras[vMapIndex]).find("CameraIndexCode").text() + "\");' onMouseUp='setCameraId(" + $(cameras[vMapIndex]).find("CameraID").text() + ");'><a href='#'>" + $(cameras[vMapIndex]).find("CameraName").text() + "</a></li>");
					}
				}
			}
		}
...
function QryRight(mask, camIdxs, updateCamArray){
	var OCXobj = document.getElementById("PreviewOcx");
	var vRightRet = OCXobj.QueryRightByIndexCodes(0x0080, camIdxs);
	if (vRightRet == null){
		//alert("批量查询预览权限失败！");
		return ;
	}
...
function startPreview(cameraId) {
	var OCXobj = document.getElementById("PreviewOcx");
	var rightCfg = document.getElementById("rightSelId").value;
	var cameraIndex = document.getElementById("CamerIndexCode").value;
	if (rightCfg == 1){
		var vRet = OCXobj.QueryRightByIndexCode(128,cameraIndex);
		if (vRet == 1){
			//alert("查询预览权限失败")
			return ;
		}
